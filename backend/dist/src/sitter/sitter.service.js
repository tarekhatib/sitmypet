...
let SitterService = class SitterService {
    ...
    async explore(query, userId) {
        const { search, services, location, sortBy, minRating, page = 1, limit = 20, } = query;
        const profile = await this.prisma.profile.findUnique({
            where: { userId },
            include: { location: true },
        });
        ...
        if (sortBy === explore_query_dto_1.SortBy.HIGHEST_RATED ||
            sortBy === explore_query_dto_1.SortBy.RATING_HIGH_TO_LOW) {
            processedPosts.sort((a, b) => b.avgRating - a.avgRating);
        }
        else if (sortBy === explore_query_dto_1.SortBy.MOST_REVIEWS) {
            processedPosts.sort((a, b) => b.reviewCount - a.reviewCount);
        }
        else if (sortBy === explore_query_dto_1.SortBy.LOWEST_PRICE ||
            sortBy === explore_query_dto_1.SortBy.PRICE_LOW_TO_HIGH) {
            processedPosts.sort((a, b) => (Number(a.price) || 0) - (Number(b.price) || 0));
        }
        else if (sortBy === explore_query_dto_1.SortBy.HIGHEST_PRICE ||
            sortBy === explore_query_dto_1.SortBy.PRICE_HIGH_TO_LOW) {
            processedPosts.sort((a, b) => (Number(b.price) || 0) - (Number(a.price) || 0));
        }
        else if (sortBy === explore_query_dto_1.SortBy.NEAREST_FIRST) {
            processedPosts.sort((a, b) => {
                if (a.isNear && !b.isNear)
                    return -1;
                if (!a.isNear && b.isNear)
                    return 1;
            ...
            processedPosts.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
        }
