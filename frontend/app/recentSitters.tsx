    ...
    useEffect(() => {
        const getBookings = async () => {
            ...
            try {
                ...
                setClients(
                  [...res.data.recentSitters].sort((a: any, b: any) => {
            ...
            {loading ? (
              <View className="flex mt-10">
                ...
                <FlatList
                    ...
                    renderItem={({item}) => (
                        <TouchableOpacity className={"w-full px-10 mb-6 flex flex-row h-20"} onPress={() => router.push(`/users/${item.id}`)}>
                            <View>
                                <Image
                                    source={{
                                        uri: item.sitterImageUrl
                                    }}
                                    className="w-20 h-20 rounded-full mr-8"
                                />
                            </View>
                            <View className={"flex flex-col justify-around items-start flex-1"}>
                                <Text className={"font-bold text-lg mt-2"}>{item.sitterName}</Text>
